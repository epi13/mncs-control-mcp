...
The Journal Context surface consists of `journal_context_status`,
`journal_context_collect`, and `journal_context_get`. It creates an immutable,
paginated local MNCS evidence bundle for an explicit interval, including
local-only Git work, uncommitted state, durable experiments, public
Commons/Fabric/Forge references, redacted Control activity, and configured
local notes. Source completeness is explicit; the projection remains evidence
and never becomes the journal author. See [docs/ATLAS-JOURNAL-INTEGRATION.md](docs/ATLAS-JOURNAL-INTEGRATION.md).
