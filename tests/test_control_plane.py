...
def test_named_workflow_has_bounded_auditable_steps(config) -> None:
    ...
    assert [step["operation"] for step in result["steps"]] == ["project_review", "test_discover", "project_check"]
...
def test_external_control_job_has_stable_lifecycle(config) -> None:
    ...
    while manager.status(job["job_id"])["status"] in {"queued", "running"} and time.monotonic() < deadline:
        time.sleep(0.01)
...
@pytest.mark.parametrize(
    ("operation", "expected"),
    [(lambda: (_ for _ in ()).throw(RuntimeError("upstream boom")), "failed"), (lambda: time.sleep(1), "timed_out")],
)
def test_external_jobs_have_truthful_exception_and_deadline_states(config, operation, expected) -> None:
    policy = WorkspacePolicy(config)
...
def test_external_cancellation_before_execution_and_while_owned_are_distinct(config) -> None:
    ...
    second = manager.submit_external("fabric_test", lambda: {"status": "should-not-run"}, timeout_seconds=2)
    stopped = manager.stop_control(second["job_id"])
    assert stopped["status"] == "stopped"
    deadline = time.monotonic() + 2
    while manager.status(first["job_id"])["status"] in {"queued", "running"} and time.monotonic() < deadline:
        time.sleep(0.01)
...
def test_external_capacity_recovers_after_timeout_and_restart_is_truthful(config) -> None:
    ...
    deadline = time.monotonic() + 3
    while manager.status(timed_out["job_id"])["status"] in {"queued", "running"} and time.monotonic() < deadline:
        time.sleep(0.01)
    replacement = manager.submit_external("fabric_test", lambda: {"status": "completed"}, timeout_seconds=1)
    deadline = time.monotonic() + 2
    while manager.status(replacement["job_id"])["status"] in {"queued", "running"} and time.monotonic() < deadline:
        time.sleep(0.01)
