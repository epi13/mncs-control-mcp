...
def _read_json_response(
    stream: Any, process: subprocess.Popen[str], request_id: int, timeout: float
) -> dict[str, Any]:
...
def probe_mcp_stdio(
    executable: Path, config_path: Path, *, timeout: float = 20.0
) -> dict[str, Any]:
...
def _service_state() -> tuple[str, str]:
    ...
    enabled_code, enabled = _capture(
        ["systemctl", "--user", "is-enabled", "mncs-control-tunnel.service"]
    )
    active_code, active = _capture(
        ["systemctl", "--user", "is-active", "mncs-control-tunnel.service"]
    )
    if active_code == 0:
...
def _tunnel_process_running(profile: str) -> bool:
    ...
    try:
        for entry in Path("/proc").iterdir():
            ...
            try:
                command = (
                    (entry / "cmdline").read_bytes().replace(b"\x00", b" ").decode(errors="ignore")
                )
            except OSError:
                continue
            if (
                "tunnel-client" in command
                and " run " in f" {command} "
                and f"--profile {profile}" in command
            ):
                return True
    except OSError:
        pass
...
def _ssh_state() -> tuple[str, str]:
    ...
    code, output = _capture(
        ["ssh-add", "-l"], env=safe_host_probe_environment({"SSH_AUTH_SOCK": str(socket)})
    )
    if code == 0:
...
def run_doctor(
    config_path: Path, *, profile: str = "mncs-fedora", json_output: bool = False
) -> int:
    ...
    try:
        ...
        if workspace.is_dir() and os.access(workspace, os.W_OK):
            ...
            checks.append(
                Check("Workspace", "FAIL", f"missing or not writable: {workspace}", required=True)
            )
    ...
    checks.append(
        Check(
            "Bubblewrap",
            "OK",
            f"{bwrap} ({_first_line(_capture([bwrap, '--version'])[1])})",
            required=True,
        )
        if bwrap
        else Check(
            "Bubblewrap",
            "FAIL",
            "not found; install with: sudo dnf install bubblewrap",
            required=True,
        )
    )
    ...
    if paths.mcp_executable.is_file() and os.access(paths.mcp_executable, os.X_OK):
        ...
        checks.append(
            Check("MCP executable", "FAIL", f"missing: {paths.mcp_executable}", required=True)
        )
    ...
    if tunnel.is_file() and os.access(tunnel, os.X_OK):
        ...
        if code == 0:
            checks.append(
                Check("Tunnel client", "OK", f"{tunnel} ({_first_line(version)})", required=True)
            )
        else:
            checks.append(
                Check("Tunnel client", "FAIL", f"{tunnel} did not pass --version", required=True)
            )
    else:
        tunnel = None
        checks.append(
            Check(
                "Tunnel client",
                "FAIL",
                "not found; install the official binary from Platform tunnel settings or the latest openai/tunnel-client release",
                required=True,
            )
        )
    ...
    key_mode_ok = (
        paths.tunnel_environment.is_file()
        and (paths.tunnel_environment.stat().st_mode & 0o777) == 0o600
    )
    if key_configured and key_mode_ok:
        checks.append(
            Check(
                "Runtime key",
                "OK",
                f"configured in {paths.tunnel_environment} (value hidden)",
                required=True,
            )
        )
    elif key_configured:
        checks.append(
            Check(
                "Runtime key",
                "FAIL",
                f"configured but permissions are not 0600: {paths.tunnel_environment}",
                required=True,
            )
        )
    else:
        checks.append(
            Check(
                "Runtime key",
                "FAIL",
                f"CONTROL_PLANE_API_KEY is not configured in {paths.tunnel_environment}",
                required=True,
            )
        )
    ...
    if tunnel is not None and key_configured:
        code, output = _capture(
            ...
            timeout=30,
            env=runtime_environment(paths.tunnel_environment),
            max_chars=8000,
        )
        if code == 0:
            checks.append(
                Check(
                    "Tunnel profile",
                    "OK",
                    f"{profile}; {_first_line(output) or 'doctor passed'}",
                    required=True,
                )
            )
        elif (
            "health_listener" in output
            and "address already in use" in output
            and _tunnel_process_running(profile)
        ):
            checks.append(
                Check(
                    "Tunnel profile",
                    "OK",
                    f"{profile}; an existing tunnel-client already owns the health listener",
                    required=True,
                )
            )
        else:
            checks.append(
                Check(
                    "Tunnel profile",
                    "FAIL",
                    f"{profile}; {_first_line(output) or 'doctor failed'}",
                    required=True,
                )
            )
    else:
    ...
    if config is not None:
        if config.fabric_mode in {"service", "transitional"}:
            ...
            try:
                ...
                contract = (
                    client.contract()
                    if client and callable(getattr(client, "contract", None))
                    else {}
                )
                features = contract.get("features", {}) if isinstance(contract, dict) else {}
                fabric_api_ok = (
                    callable(getattr(client, "connect", None))
                    and features.get("persistent_fleet_read") is True
                )
            ...
            checks.append(
                Check(
                    ...
                    f"{socket_path} (AF_UNIX consumer socket)"
            ...
            try:
                ...
                checks.append(
                    Check(
                        ...
                        "persistent controller responded"
                        if connected
                        else str(fabric_status.get("diagnostic", "unavailable")),
                        required=True,
                    )
                )
                checks.append(
                    Check(
                        ...
                        "OK"
                        if connected
                        and fabric_status.get("persistent_service_support", {}).get(
                ...
                checks.append(
                    Check("Fabric controller", "FAIL", redact_text(str(exc)), required=True)
                )
            checks.append(
                Check(
                    "Fabric admin authority",
                    "OK",
                    "Control uses ordinary consumer access; admin socket is not configured",
                    required=True,
                )
            )
            checks.append(
                Check(
                    "Fabric worker credentials",
                    "OK",
                    "consumer mode does not require worker certificates or private keys",
                    required=True,
                )
            )
            checks.append(
                Check(
                    "Remote connector",
                    "UNKNOWN",
                    "external ChatGPT-side verification required; local checks do not establish connector reachability",
                )
            )
        else:
            ...
            checks.append(
                Check(
                    "Fabric registry",
                    "OK" if registry_path.is_file() else "WARNING",
                    str(registry_path),
                )
            )
        checks.append(
            Check(
                "Harness",
                "OK" if config.harness_path.is_dir() else "WARNING",
                str(config.harness_path),
            )
        )
        checks.append(
            Check(
                "Forge", "OK" if config.forge_path.is_dir() else "WARNING", str(config.forge_path)
            )
        )
        ...
        if nvidia:
            code, output = _capture(
                [nvidia, "--query-gpu=name,driver_version", "--format=csv,noheader"], timeout=8
            )
            checks.append(
                Check(
                    "CUDA",
                    "OK" if code == 0 else "WARNING",
                    _first_line(output) if code == 0 else "nvidia-smi unavailable",
                )
            )
        else:
    ...
    if config is not None:
        ...
        try:
            ...
            integrations = IntegrationBundle(config, ActionRegistry(), policy, sandbox)
        except Exception:
            sandbox = None
            integrations = None
        developer = developer_readiness_payload(config, sandbox=sandbox, integrations=integrations)
        status_for = {
            "available": "OK",
            "optional": "WARNING",
            "degraded": "WARNING",
            "unknown": "UNKNOWN",
            "authenticated_insufficient": "FAIL",
            "misconfigured": "FAIL",
            "unavailable": "FAIL",
        }
        for item in developer["checks"]:
            checks.append(
                Check(
                    str(item["name"]),
                    status_for.get(str(item["state"]), "WARNING"),
                    str(item["detail"]),
    ...
    if json_output:
        ...
        if developer is not None:
            print(
                "Development ready           "
                + ("YES" if developer.get("ready_for_development") else "NO")
            )
            print(
                "Pull request ready          "
                + ("YES" if developer.get("ready_for_pull_request") else "NO")
            )
...
def doctor_main(argv: list[str] | None = None) -> int:
    ...
    return run_doctor(
        Path(args.config).expanduser().resolve(), profile=args.profile, json_output=args.json
    )
